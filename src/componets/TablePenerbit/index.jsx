...
import style from "./tabelPengarang.module.css";
import { InputText } from "primereact/inputtext";
import { FilterMatchMode } from "primereact/api";
import ModelDelete from "../Model/ModelMhs/ModelMhsDelete";
import { getPengarang } from "../../config/redux/actions/pengarangAction";
import ModelUpdatePengarang from "../Model/ModelPengarang/ModelPengarangUpdale";
import { getPenerbit } from "../../config/redux/actions/penerbitAction";

function TblPenerbit() {
  ...
  useEffect(() => {
    dispatch(getPenerbit(setData, setLoading));
    console.log(data);
  }, []);
  ...
  let actionBodyTamplate = (data) => {
    return (
      <>
        <ModelUpdatePengarang id_pengarang={data.id_pengarang} nama={data.nama} alamat={data.alamat} telepon={data.telepon} >
          ...
          <span> Upadte</span>
        </ModelUpdatePengarang>
        <ModelDelete className="ml-2" id_pengarang={data.id_pengarang} nama={data.nama}>
        ...
        <div className="col-12">
          <div className={`row inputEdit justify-content-center mb-3 mb-md-5 ${style.themeRow} ${style.rowOneRight}`}>
            <div className="col-11">
              <h3 className="pt-4">Pengarang</h3>
                ...
                <table className={`${style.table} table table-responsive-sm `}>
                  <tbody>
                    <div className={`card ${style.card}`}>
                      <DataTable value={data} paginator rows={5} loading={loading} showGridlines rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: "30rem", position: "center" }}>
                        ...
                        <Column field="telepon" header="Nomor Telepon" sortable style={{ width: "25%" }}></Column>
